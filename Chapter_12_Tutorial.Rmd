...
hyper(?)link learning outcomes to each section
add terms to learning outcomes, 
make clear what each piece will be for later
some more context, 
...
* [Useful terms for working with Git and GitHub](#useful-terms-for-working-with-git-and-github)
...
###~~~
gitterms <- read.csv("Chapter_1_B_National_Academies_Report_Recommendations.csv")
...
2. Next, select the Git button to clone a project from a Git repository.

```{r GitRproject, echo=FALSE, fig.align= 'center', out.width = '50%'}
knitr::include_graphics("Pictures/GitRproject.jpg")
```

3. Go back to your GitHub, find the repository you just made and go to the green Code button in the upper right hand corner. Click this and a tab will appear that says Clone. Under this tab, there is a HTTPS section. Make sure you are in this section, then press the icon of one square on top of another inside of the red circle below in Figure \@ref(fig:redcircle).
...
```{r activity6, echo=FALSE, out.width = '100%'}
...
* Do not commit at the same time as your collaborator, Git does not like it. It will break show an error Figure \@ref(fig:redgreen)
...
Many scientists have struggled to reproduce their own lab's work as well as work from other labs [@Baker_2016]. However, the survey in @Baker_2016 points to several factors that may contribute to this phenomenon and what may be changed to solve it.
